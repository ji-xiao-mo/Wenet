...
# Windows compatibility: use Python directly instead of torchrun
USE_TORCHRUN=false
...
# The aishell dataset location root. You can override with:
#   bash run.sh --data /your/path --stage 0 --stop_stage 4
data=/home/a631/xzy/WeNet
...
#    autograd error
train_config=conf/train_conformer_adam.yaml
dir=exp/conformer_adam
tensorboard_dir=tensorboard_adam
checkpoint=
num_workers=4
prefetch=10
...
for root in "${search_roots[@]}"; do
  if [ -d "${root}/data_aishell/data_aishell/wav" ] && [ -d "${root}/data_aishell/data_aishell/transcript" ]; then
    aishell_wav_dir="${root}/data_aishell/data_aishell/wav"
    aishell_transcript_dir="${root}/data_aishell/data_aishell/transcript"
    break
  fi
  if [ -d "${root}/data_aishell/wav" ] && [ -d "${root}/data_aishell/transcript" ]; then
    aishell_wav_dir="${root}/data_aishell/wav"
    aishell_transcript_dir="${root}/data_aishell/transcript"
    break
  fi
done

for root in "${search_roots[@]}"; do
  if [ -f "${root}/resource_aishell/resource_aishell/lexicon.txt" ]; then
    aishell_lexicon="${root}/resource_aishell/resource_aishell/lexicon.txt"
    break
  fi
  if [ -f "${root}/resource_aishell/lexicon.txt" ]; then
    aishell_lexicon="${root}/resource_aishell/lexicon.txt"
    break
  fi
done

if [ -z "${aishell_wav_dir}" ] || [ -z "${aishell_transcript_dir}" ]; then
  echo "Cannot find AISHELL wav/transcript. searched roots: ${search_roots[*]}" && exit 1
fi

if [ -z "${aishell_lexicon}" ]; then
  echo "Cannot find AISHELL lexicon under searched roots: ${search_roots[*]}" && exit 1
fi

echo "Using AISHELL wav dir: ${aishell_wav_dir}"
echo "Using AISHELL transcript dir: ${aishell_transcript_dir}"
echo "Using AISHELL lexicon: ${aishell_lexicon}"
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
  # Data preparation
  bash local/aishell_data_prep.sh ${aishell_wav_dir} \
    ${aishell_transcript_dir}
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  echo "<sos/eos> 2" >> $dict
  python tools/text2token.py -s 1 -n 1 data/train/text | cut -f 2- -d" " \
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  if [ "${USE_TORCHRUN}" = "false" ]; then
    # Windows compatibility: use Python directly with environment variables
    export RANK=0
    export LOCAL_RANK=0
    export WORLD_SIZE=1
    ...
    # Only add deepspeed args if using deepspeed
    deepspeed_args=""
    if [ ${train_engine} == "deepspeed" ]; then
      deepspeed_args="--deepspeed_config ${deepspeed_config} --deepspeed.save_states ${deepspeed_save_states}"
    fi
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
  ...
  python tools/fst/prepare_dict.py $unit_file ${aishell_lexicon} \
  ...
  bash local/aishell_train_lms.sh
