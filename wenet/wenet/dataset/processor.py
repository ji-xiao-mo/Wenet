...
_SOX_EFFECTS_UNAVAILABLE_WARNED = False
...
def speed_perturb(sample, speeds=None):
    ...
    if speed != 1.0:
        global _SOX_EFFECTS_UNAVAILABLE_WARNED
        if not hasattr(torchaudio, 'sox_effects') or \
                not hasattr(torchaudio.sox_effects, 'apply_effects_tensor'):
            if not _SOX_EFFECTS_UNAVAILABLE_WARNED:
                logging.warning('torchaudio.sox_effects is unavailable, '
                                'speed_perturb will be skipped.')
                _SOX_EFFECTS_UNAVAILABLE_WARNED = True
            return sample
