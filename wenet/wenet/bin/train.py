...
def main():
    ...
    if final_epoch is not None and rank == 0:
        ...
        import shutil
        # Check if the file exists with 'epoch_X.pt' format
        src_path_epoch = os.path.join(args.model_dir, 'epoch_{}.pt'.format(final_epoch))
        src_path_num = os.path.join(args.model_dir, '{}.pt'.format(final_epoch))
        if os.path.exists(src_path_epoch):
            src_path = src_path_epoch
        elif os.path.exists(src_path_num):
            src_path = src_path_num
        else:
            src_path = src_path_epoch  # This will raise a clear error
        shutil.copy(src_path, final_model_path)
        writer.close()
    dist.barrier(
