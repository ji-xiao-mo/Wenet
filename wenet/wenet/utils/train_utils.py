...
def init_distributed(args):
    ...
    if args.train_engine in ["torch_ddp", "torch_fsdp"]:
        if "cuda" in args.device:
            ...
            pass  # CPU device, no need to set
        else:
            logging.error("not supported device: {}".format(args.device))

        # Skip distributed init for single-process training
        if world_size == 1 and args.device in ["cpu", "cuda", "npu"]:
            logging.info(
                "Skipping distributed initialization for single-process {} training".format(
                    args.device))
            # Mock the dist module functions that will be called
            import torch.distributed as dist
            if not hasattr(dist, '_wenet_mocked'):
                dist._wenet_mocked = True

                def mock_barrier(*args, **kwargs):
                    pass

                def mock_all_reduce(*args, **kwargs):
                    pass

                def mock_new_group(*args, **kwargs):
                    return dist.GroupMember.NON_GROUP_MEMBER

                def mock_destroy_group(*args, **kwargs):
                    pass

                def mock_monitored_barrier(*args, **kwargs):
                    pass

                def mock_broadcast(*args, **kwargs):
                    pass

                def mock_all_gather(*args, **kwargs):
                    pass

                def mock_reduce_scatter(*args, **kwargs):
                    pass

                dist.barrier = mock_barrier
                dist.all_reduce = mock_all_reduce
                dist.new_group = mock_new_group
                dist.destroy_process_group = mock_destroy_group
                dist.monitored_barrier = mock_monitored_barrier
                dist.broadcast = mock_broadcast
                dist.all_gather = mock_all_gather
                dist.reduce_scatter = mock_reduce_scatter
        else:
...
def wrap_cuda_model(args, model, configs=None):
    ...
    if args.train_engine == "torch_ddp":  # native pytorch ddp
        ...
        model.to(device)
        # Skip DDP for single-process CPU/CUDA training on Windows
        if world_size == 1 and args.device in ["cpu", "cuda"]:
            logging.info("Skipping DDP for single-process {} training".format(args.device))
        else:
...
def init_optimizer_and_scheduler(args, configs, model):
    ...
    elif configs['optim'] == 'cautious_muon':
        from wenet.optimizer.muon import CautiousMuon
        optimizer = CautiousMuon(params, **optim_conf)
    elif configs['optim'] == 'pogo':
        import sys
        import os
        pogo_path = os.path.join(os.path.dirname(__file__), '..', 'optimizer',
                                 'pogo-main', 'pogo-main')
        if not os.path.isdir(pogo_path):
            raise FileNotFoundError(
                "POGO optimizer not found at: {}. Ensure the POGO source is "
                "present under wenet/optimizer/pogo-main/pogo-main.".format(
                    pogo_path))
        if pogo_path not in sys.path:
            sys.path.insert(0, pogo_path)
        try:
            from pogo import POGO
        except Exception as exc:
            raise ImportError(
                "Failed to import POGO from {}. Check the POGO package layout "
                "and dependencies.".format(pogo_path)) from exc
        # POGO requires base_optimizer class (not instance) and lr parameter
        pogo_lr = optim_conf.pop('lr', 0.001)
        optimizer = POGO(params, base_optimizer=optim.Adam, lr=pogo_lr,
                         **optim_conf)
    else:
...
def log_per_epoch(writer, info_dict):
    ...
    rank = int(os.environ.get('RANK', 0))
    step = info_dict["step"]
    cv_loss = loss_dict.get("loss", loss_dict.get("cv_loss", 0.0))
    cv_loss_str = tensor_to_scalar(cv_loss) if cv_loss is not None else "N/A"
    logging.info(
        'Epoch {} Step {} CV info lr {} cv_loss {} rank {} acc {}'.format(
            epoch, step, lrs_to_str(lrs), cv_loss_str,
